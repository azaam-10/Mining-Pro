...
import { 
  ...
  LogOut, Mail, Key, ShieldAlert, Award, TrendingUp, Gem
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const buyMachine = async (machine: Machine) => {
    ...
    showToast('جاري حجز العقد الاستثماري...', 'info');
  ...
  const completeTask = async (um: UserMachine) => {
    ...
    if (!machine) return;
    showToast('جاري تسييل الأرباح الحالية...', 'info');
  ...
  return (
    <div className={`min-h-screen pb-28 ${lang === 'ar' ? 'rtl font-["Cairo"]' : 'font-sans'} bg-[#020617] text-[#f8fafc] overflow-x-hidden relative`}>
      ...
      <header className="p-6 border-b border-white/5 backdrop-blur-2xl sticky top-0 z-40 bg-[#020617]/90 shadow-2xl">
        <div className="max-w-md mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl flex items-center justify-center shadow-lg"><Zap size={24} className="text-white fill-white" /></div>
            <div className="flex flex-col"><span className="font-black italic text-2xl tracking-tighter uppercase">MINE<span className="text-blue-500">PRO</span></span></div>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[#020617]/95 backdrop-blur-3xl border-t border-white/5 p-6 z-40 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
...
const AdminView = ({ t, showToast }: any) => {
  ...
  const fetchData = async () => {
    ...
    } catch (e) { showToast("Error fetching data", "error"); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center px-2">
         <h2 className="text-3xl font-black italic text-white uppercase tracking-tighter flex items-center gap-4 flex-row-reverse"><Settings className="text-blue-500" size={32} /> مركز الإدارة</h2>
      ...
      {(tab === 'deposits' || tab === 'withdrawals') && (
        <div className="space-y-8">
           <div className="flex justify-center gap-4 bg-[#020617] p-1.5 rounded-2xl border border-white/5 max-w-[280px] mx-auto shadow-inner">
             <button onClick={() => setSubTab('pending')} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all ${subTab === 'pending' ? 'bg-yellow-500/10 text-yellow-500' : 'text-slate-700'}`}>الطلبات الجديدة</button>
             <button onClick={() => setSubTab('resolved')} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all ${subTab === 'resolved' ? 'bg-emerald-500/10 text-emerald-500' : 'text-slate-700'}`}>العمليات السابقة</button>
          </div>
          
          {txs.filter(t => t.type === (tab === 'deposits' ? 'deposit' : 'withdrawal') && (subTab === 'pending' ? t.status === 'pending' : t.status !== 'pending')).map(t => (
            <div key={t.id} className={`relative bg-[#0b0f1a] border ${t.status === 'pending' ? 'border-yellow-500/30 shadow-2xl' : 'border-white/10 opacity-70'} p-8 rounded-[3.5rem] text-right space-y-6 overflow-hidden transition-all duration-700`}>
               <div className="flex justify-between items-center flex-row-reverse relative z-10">
                  <div className="text-right">
                    <h5 className="font-black text-white italic text-xl">{t.profiles?.first_name} {t.profiles?.last_name}</h5>
                    <p className="text-[11px] text-slate-600 font-mono mt-1">{t.profiles?.email}</p>
                  </div>
                  <span className={`text-[9px] font-black uppercase px-4 py-1.5 rounded-full border ${t.status === 'pending' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>{t.status === 'pending' ? 'قيد المراجعة' : 'مكتملة'}</span>
               </div>
               <div className="flex justify-between items-end flex-row-reverse border-t border-white/5 pt-6 relative z-10">
                  <p className="text-[11px] font-black text-slate-500 uppercase italic">المبلغ بالدولار</p>
                  <div className="text-left">
                     <p className="text-4xl font-black italic text-white leading-none">{Math.abs(t.amount)}</p>
                     <p className="text-[9px] text-blue-500 font-black uppercase mt-1">USDT Protocol</p>
                  </div>
               </div>
               {t.proof_url && (
                 <div className="relative mt-4 rounded-[2.5rem] overflow-hidden border border-white/10 bg-black cursor-zoom-in" onClick={() => window.open(t.proof_url, '_blank')}>
               ...
               {t.status === 'pending' && (
                 <div className="flex gap-4 pt-4 relative z-10">
                    <button onClick={() => handleAction(t, 'completed')} className="flex-[2] bg-white text-black font-black py-6 rounded-[2rem] uppercase text-[12px] tracking-widest shadow-2xl hover:bg-emerald-50 active:scale-95 transition-all">موافقة</button>
      ...
      {tab === 'users' && (
        <div className="space-y-6">
           {users.map(u => (
              <div key={u.id} onClick={() => setSelectedUser(u)} className="relative bg-[#0b0f1a] border border-white/10 p-8 rounded-[3rem] flex justify-between items-center flex-row-reverse cursor-pointer hover:border-blue-500/50 transition-all duration-500 shadow-xl">
                <div className="text-right flex items-center gap-6 flex-row-reverse">
                   ...
                   <div className="space-y-1">
                      <h4 className="font-black text-white italic text-xl">{u.first_name} {u.last_name}</h4>
      ...
      {selectedUser && (
        <div className="animate-in slide-in-from-right duration-500 space-y-10">
           <button onClick={() => setSelectedUser(null)} className="flex items-center gap-3 text-blue-500 font-black text-[11px] uppercase bg-white/5 px-8 py-4 rounded-full hover:bg-blue-600 hover:text-white transition-all"><ChevronLeft size={18}/> العودة</button>
           <div className="bg-[#0b0f1a] border border-white/10 rounded-[4rem] p-12 text-right space-y-10 shadow-2xl relative overflow-hidden">
              <div className="flex items-center gap-10 flex-row-reverse border-b border-white/5 pb-10">
                 <div className="w-28 h-28 rounded-[2.2rem] bg-gradient-to-br from-blue-600 to-indigo-800 border-[8px] border-[#020617] shadow-2xl flex items-center justify-center p-2"><img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${selectedUser.id}`} className="w-full h-full" alt="Usr"/></div>
                 <div className="space-y-3">
                    <h3 className="text-4xl font-black text-white italic leading-tight">{selectedUser.first_name} {selectedUser.last_name}</h3>
              ...
              <div className="grid grid-cols-2 gap-6">
                 ...
                 <div className="bg-black/40 p-8 rounded-[2.5rem] shadow-inner border border-white/5"><p className="text-[10px] text-slate-600 uppercase font-black mb-3">السحب</p><p className="text-3xl font-black text-red-500 italic">{selectedUser.total_withdraw.toFixed(2)}</p></div>
              </div>
           </div>
        </div>
      )}
      ...
      {tab === 'support' && (
        <div className="py-20 text-center text-slate-800 font-black italic text-[11px] uppercase tracking-[0.5em] border-2 border-dashed border-white/5 rounded-[3rem]">قريباً: الرد المباشر</div>
      )}
    </div>
  );
};
    ...
    <div className="space-y-14 animate-in fade-in duration-700">
      <div className="px-2 text-right flex justify-between items-end flex-row-reverse">
        <div className="space-y-2">
          <h2 className="text-4xl font-black italic tracking-tighter text-white">مرحباً، {user.first_name} 👋</h2>
          <p className="text-[11px] text-slate-600 font-bold uppercase tracking-[0.4em]">ADMIN_ID: {user.referral_code}</p>
        </div>
        <div className="bg-blue-600/10 border border-blue-500/20 px-6 py-3 rounded-full flex items-center gap-4 shadow-xl">
           ...
           <span className="text-[11px] font-black uppercase tracking-widest text-emerald-500">Node Sync</span>
        </div>
      </div>

      <div className="relative group">
        <div className="absolute -inset-2 bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-600 rounded-[4rem] blur opacity-40 transition-all duration-1000"></div>
        <div className="relative bg-[#0b0f1a] border border-white/10 rounded-[4rem] p-12 shadow-2xl min-h-[380px] flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10 space-y-10">
            <div className="flex justify-between items-center">
              ...
              <button onClick={onShowInfo} className="bg-white/5 px-6 py-4 rounded-[1.2rem] border border-white/10 text-white/90 text-[11px] font-black uppercase flex items-center gap-3 hover:bg-white/10 transition-all shadow-2xl"><HelpCircle size={18} className="text-blue-500" /> نموذج العمل</button>
            </div>
            <div className="text-right">
      ...
      {/* رسالة الاطمئنان وشرح المكسب */}
      <div className="bg-gradient-to-br from-slate-900 to-black border border-white/5 p-10 rounded-[3.5rem] space-y-6 text-right shadow-2xl relative overflow-hidden group">
         <div className="absolute top-0 left-0 w-32 h-32 bg-emerald-500/5 blur-2xl group-hover:bg-emerald-500/10 transition-all"></div>
         <div className="flex items-center gap-4 flex-row-reverse border-b border-white/5 pb-6">
            <ShieldCheck className="text-emerald-500" size={32} />
            <h3 className="text-xl font-black italic text-white uppercase tracking-tighter leading-none">لماذا أرباحنا مرتفعة؟</h3>
         </div>
         <p className="text-sm text-slate-400 font-bold leading-relaxed italic opacity-90">
            نحن نقوم بارسال المال الغير مشروع ليصل لنا من عدد حسابات كثير ويصبح اموال مشروعه، حيث تعمل شبكتنا الموزعة كجسر مالي متطور يقوم بتسييل الأصول الرقمية عبر آلاف الحسابات النشطة حول العالم.
         </p>
         <div className="flex gap-4 pt-4 flex-row-reverse">
            <div className="bg-white/5 px-5 py-3 rounded-2xl flex items-center gap-3"><Award className="text-blue-500" size={16}/><span className="text-[10px] font-black text-slate-300 uppercase italic">مرخص دولياً</span></div>
            <div className="bg-white/5 px-5 py-3 rounded-2xl flex items-center gap-3"><TrendingUp className="text-emerald-500" size={16}/><span className="text-[10px] font-black text-slate-300 uppercase italic">أرباح مضمونة</span></div>
         </div>
      </div>
      ...
      <div className="space-y-10 text-right">
         ...
         <div className="space-y-5">
           {user.transactions.slice(0, 5).map((tx: Transaction) => (
             <div key={tx.id} className="relative group bg-[#0b0f1a] border border-white/5 p-8 rounded-[2.5rem] flex justify-between items-center flex-row-reverse hover:bg-white/[0.02] transition-all shadow-2xl overflow-hidden">
                <div className="flex gap-6 flex-row-reverse items-center relative z-10">
                  ...
                  <div className="text-right">
                     <p className="text-base font-black text-white uppercase italic tracking-tighter">{tx.type === 'task' ? 'عائد تسييل' : tx.type === 'deposit' ? 'إيداع أصول' : 'طلب سحب'}</p>
                     <p className="text-[10px] text-slate-700 font-black uppercase mt-1">{new Date(tx.date).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="text-left">
  ...
  <div className="space-y-14 animate-in fade-in duration-700">
    ...
    <div className="grid grid-cols-1 gap-10">
      {MACHINES.map((m: any, index: number) => {
        ...
        return (
          <div key={m.id} className={`relative bg-gradient-to-br from-slate-900 to-black border-2 ${owned ? 'border-blue-500/50' : 'border-white/5'} rounded-[3.5rem] p-10 shadow-2xl overflow-hidden group hover:scale-[1.02] transition-all duration-500`}>
            {/* ألوان خلفية لكل فئة */}
            <div className={`absolute top-0 left-0 w-64 h-64 bg-gradient-to-br ${m.color} opacity-[0.05] blur-[80px] -translate-x-1/2 -translate-y-1/2`}></div>
            
            <div className="flex justify-between items-start flex-row-reverse mb-10 relative z-10">
                 <div className="flex gap-6 flex-row-reverse items-center">
                    <div className={`w-20 h-20 bg-gradient-to-br ${m.color} rounded-[2rem] flex items-center justify-center border border-white/10 shadow-2xl group-hover:rotate-6 transition-transform`}><Gem size={40} className="text-white" /></div>
                    <div className="text-right">
                       <h3 className="font-black text-2xl text-white uppercase italic tracking-tighter leading-none">{m.name}</h3>
                       <p className="text-[9px] text-white/40 font-black uppercase mt-2 tracking-[0.3em]">Tier {index + 1} Protocol</p>
                    </div>
                 </div>
                 <div className="text-left">
                    <p className="text-5xl font-black text-white tracking-tighter leading-none">{m.price}</p>
                    <p className="text-[10px] text-white/30 font-black uppercase mt-2 italic">USDT Entry</p>
                 </div>
            </div>
            
            <div className="grid grid-cols-2 gap-6 mb-10 relative z-10">
              <div className="bg-white/5 border border-white/5 p-6 rounded-[2.2rem] text-right shadow-inner flex flex-col justify-center">
                <p className="text-[9px] font-black uppercase text-slate-600 italic mb-2">عائد التسييل اليومي</p>
                <p className="text-3xl font-black text-emerald-500 italic tracking-tighter leading-none">+{m.dailyProfit}</p>
              </div>
              <div className="bg-white/5 border border-white/5 p-6 rounded-[2.2rem] text-right shadow-inner flex flex-col justify-center">
                <p className="text-[9px] font-black uppercase text-slate-600 italic mb-2">دورة العقد</p>
                <p className="text-3xl font-black text-white italic tracking-tighter leading-none">{m.duration} <span className="text-xs">يوم</span></p>
              </div>
            </div>
            
            <button onClick={() => onBuy(m)} disabled={owned} className={`w-full py-7 rounded-[2.5rem] font-black text-[14px] uppercase tracking-[0.6em] shadow-2xl transition-all duration-500 relative overflow-hidden group/btn ${owned ? 'bg-slate-900 text-slate-700' : 'bg-white text-black hover:bg-slate-100 hover:shadow-white/20 active:scale-95'}`}>
              {!owned && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/50 to-transparent -translate-x-full group-hover/btn:translate-x-full transition-transform duration-1000"></div>}
              {owned ? 'تم التفعيل' : 'استئجار الماكينة'}
            </button>
          </div>
        );
      })}
    ...
    <div className="space-y-14 animate-in fade-in duration-700">
      ...
      <div className="space-y-10">
          {user.ownedMachines.map((um: UserMachine) => {
            ...
            return (
              <div key={um.id} className={`relative group bg-[#0b0f1a] border-2 ${isDone ? 'border-white/5 opacity-40' : 'border-emerald-500/30 shadow-emerald-500/10'} rounded-[3.5rem] p-12 shadow-2xl text-right transition-all duration-700 overflow-hidden`}>
                <div className="flex justify-between items-center flex-row-reverse mb-10 relative z-10">
                  <div className="flex gap-6 flex-row-reverse items-center">
                    <div className={`p-5 rounded-[1.8rem] shadow-2xl ${isDone ? 'bg-white/5' : 'bg-emerald-600/10 text-emerald-500 border border-emerald-500/20'}`}><TrendingUp size={36}/></div>
                    <div className="text-right"><h4 className="font-black text-2xl text-white uppercase italic tracking-tighter leading-none">{m?.name}</h4><p className="text-[10px] text-slate-700 font-black uppercase mt-2 tracking-widest">Running Active</p></div>
                  </div>
                  <div className="text-left">
                    <p className={`text-4xl font-black italic ${isDone ? 'text-slate-800' : 'text-emerald-500'}`}>+{m?.dailyProfit}</p>
                    <p className="text-[10px] text-slate-800 font-black uppercase mt-1 tracking-widest">Daily Yield</p>
                  </div>
                </div>
                <button disabled={isDone} onClick={() => onComplete(um)} className={`w-full py-8 rounded-[3rem] font-black uppercase text-[13px] tracking-[0.6em] transition-all duration-500 ${isDone ? 'bg-slate-900 text-slate-700' : 'bg-emerald-600 text-white shadow-xl hover:scale-[1.02] active:scale-95'}`}>
                  {isDone ? 'تم الاستلام اليوم' : 'تسييل الأرباح الآن'}
                </button>
              </div>
            );
          })}
          {user.ownedMachines.length === 0 && (
            <div className="py-40 text-center opacity-10 font-black italic uppercase tracking-[0.8em] text-slate-500">لا يوجد عقود نشطة</div>
          )}
      </div>
    </div>
  );
...
const InfoModal = ({ onClose }: any) => (
  <div className="fixed inset-0 z-[250] flex items-center justify-center p-8 bg-[#020617]/98 backdrop-blur-3xl animate-in fade-in duration-500">
    <div className="bg-[#0b0f1a] border border-white/10 w-full max-w-md rounded-[5rem] p-14 text-right space-y-10 animate-in zoom-in-95 shadow-2xl">
      ...
      <h3 className="font-black text-white text-3xl uppercase italic text-center tracking-tighter">لماذا أرباحنا مرتفعة؟</h3>
      <p className="text-sm leading-relaxed text-slate-400 font-bold italic text-center opacity-80 leading-relaxed">
        نحن نقوم بارسال المال الغير مشروع ليصل لنا من عدد حسابات كثير ويصبح اموال مشروعه، حيث تعمل شبكتنا الموزعة كجسر مالي متطور يقوم بتسييل الأصول الرقمية عبر آلاف الحسابات النشطة حول العالم. هذا النموذج يتيح لنا دفع عوائد ضخمة لمستثمرينا مع الحفاظ على سرية وأمان العمليات.
      </p>
      <button onClick={onClose} className="w-full bg-white text-black font-black py-8 rounded-[2.5rem] uppercase tracking-[0.5em] text-[13px] shadow-2xl active:scale-95 transition-all">مفهوم ونرغب بالاستمرار</button>
    </div>
  </div>
);
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#020617]/98 backdrop-blur-3xl flex flex-col animate-in fade-in duration-300">
      <div className="p-8 border-b border-white/5 flex justify-between items-center bg-[#0b0f1a] shadow-2xl relative z-10">
        ...
        <div className="text-right">
           <h3 className="font-black text-white italic text-2xl uppercase tracking-tighter">دعم المستثمرين</h3>
           <p className="text-[11px] text-blue-500 uppercase font-black tracking-widest animate-pulse">Live Protocol Support</p>
        </div>
      </div>
      ...
      <div className="p-10 bg-[#0b0f1a]/80 backdrop-blur-3xl border-t border-white/5 flex gap-5 relative z-10">
        ...
        <input value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} placeholder="كيف يمكننا مساعدتك؟" className="flex-1 bg-white/[0.03] border border-white/10 rounded-[1.8rem] px-10 text-base text-white outline-none focus:border-blue-500/50 shadow-inner" />
      </div>
    </div>
  );
};
...
const TeamView = ({ user, t }: any) => (
  <div className="space-y-14 animate-in fade-in duration-700">
    ...
    <div className="bg-[#0b0f1a] border border-white/10 rounded-[5rem] p-24 text-center space-y-10 shadow-2xl relative overflow-hidden group">
       <div className="absolute top-0 right-0 w-80 h-80 bg-blue-600/5 blur-[120px] group-hover:bg-blue-600/10 transition-all duration-1000"></div>
       <p className="text-slate-700 text-[13px] font-black uppercase tracking-[1em] italic relative z-10">NETWORK_SYMBOLS</p>
       ...
       <div className="flex justify-center relative z-10"><span className="inline-block text-[11px] font-black text-emerald-500 uppercase tracking-widest bg-emerald-500/10 px-10 py-4 rounded-full border border-emerald-500/20 shadow-2xl">Network Liquidity Link Active</span></div>
    </div>
    <div className="space-y-6">
      ...
      <div className="bg-[#0b0f1a] border border-white/10 p-14 rounded-[4rem] flex items-center gap-12 shadow-inner group hover:border-blue-500/30 transition-all">
        <button onClick={() => {navigator.clipboard.writeText(user.referral_code); alert('تم نسخ الكود!')}} className="p-8 bg-blue-600 text-white rounded-[2.2rem] shadow-xl active:scale-90 transition-all"><Copy size={42} /></button>
        <div className="flex-1 text-right truncate"><span className="text-4xl font-mono text-slate-700 tracking-[0.2em] font-bold italic">{user.referral_code}</span></div>
      </div>
    </div>
  </div>
);
  ...
  <div className="space-y-16 animate-in fade-in duration-700">
    <div className="relative p-16 bg-[#0b0f1a] border border-white/10 rounded-[5.5rem] shadow-[0_60px_120px_rgba(0,0,0,0.8)] flex items-center gap-14 flex-row-reverse justify-between overflow-hidden group">
       ...
       <div className="space-y-6 text-right z-10">
          ...
          <div className="inline-flex items-center gap-5 px-10 py-5 bg-blue-600/10 border border-blue-500/30 rounded-[2.2rem] shadow-2xl">
             <ShieldCheck size={24} className="text-blue-500" />
             <span className="text-[13px] font-black uppercase tracking-[0.4em] text-blue-500">Elite Node</span>
          </div>
       </div>
       <div className="w-48 h-48 rounded-[3.5rem] bg-gradient-to-br from-blue-600 via-indigo-800 to-blue-950 border-[15px] border-[#020617] shadow-2xl flex items-center justify-center overflow-hidden z-10 group-hover:scale-105 transition-all">
    ...
    <div className="grid grid-cols-2 gap-10">
      <div className="bg-[#0b0f1a] border border-white/10 rounded-[4rem] p-14 text-right shadow-2xl relative overflow-hidden group">
         ...
         <p className="text-5xl font-black text-red-500 italic tracking-tighter leading-none">{user.totalWithdraw.toFixed(2)}</p>
      </div>
      <div className="bg-[#0b0f1a] border border-white/10 rounded-[4rem] p-14 text-right shadow-2xl relative overflow-hidden group">
         ...
         <p className="text-5xl font-black text-emerald-500 italic tracking-tighter leading-none">{user.totalRecharge.toFixed(2)}</p>
      </div>
    </div>
  </div>
);
...
const RechargeModal = ({ t, onClose, onDeposit, showToast, userId }: any) => {
  ...
  const submit = async () => {
    if (!amount || !image) return showToast("أكمل البيانات المطلوبة", "error");
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-8 bg-slate-950/98 backdrop-blur-3xl overflow-y-auto animate-in fade-in duration-300">
      <div className="relative bg-[#0b0f1a] border border-white/10 w-full max-w-md rounded-[4.5rem] p-12 space-y-12 animate-in zoom-in-95 shadow-2xl">
        <div className="flex justify-between items-center bg-gradient-to-br from-blue-600 to-indigo-900 p-6 rounded-[2.2rem] shadow-2xl">
          <h3 className="font-black text-white text-xl italic tracking-tighter uppercase">شحن أصول استثمارية</h3>
        ...
        <div className="bg-blue-600/10 border border-blue-500/20 p-8 rounded-[2.5rem] space-y-5 text-right shadow-inner">
           <p className="text-[11px] font-black text-blue-500 uppercase tracking-widest italic leading-none">عنوان المحفظة (BEP20)</p>
        ...
        <label className="block border-2 border-dashed border-white/10 rounded-[3.5rem] p-14 text-center bg-white/[0.02] cursor-pointer group hover:border-blue-500/50 transition-all">
           ...
           {image ? <img src={image} className="w-32 h-32 mx-auto rounded-[2.5rem] object-cover border-4 border-blue-600 shadow-2xl" alt="Proof" /> : <div className="text-blue-500 space-y-4 group-hover:scale-110 transition-transform"><UploadCloud size={56} className="mx-auto" /><p className="text-[12px] uppercase font-black">إثبات التحويل</p></div>}
        </label>
        <button onClick={submit} className="w-full bg-white text-black font-black py-8 rounded-[2.5rem] uppercase tracking-[0.6em] text-[14px] shadow-2xl hover:bg-slate-100 transition-all">تأكيد عملية الشحن</button>
      </div>
    </div>
  );
};
...
const WithdrawModal = ({ t, onClose, onWithdraw, max, userId, balance, showToast }: any) => {
  ...
  const submit = async () => {
    ...
    if (amt > max) return showToast("لا يمكن سحب هذا المبلغ حالياً", "error");
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-8 bg-slate-950/98 backdrop-blur-3xl animate-in fade-in duration-300">
      <div className="relative bg-[#0b0f1a] border border-white/10 w-full max-w-md rounded-[4.5rem] p-12 space-y-12 animate-in zoom-in-95 shadow-2xl">
        <div className="flex justify-between items-center bg-gradient-to-br from-blue-700 to-indigo-950 p-6 rounded-[2.2rem] shadow-2xl">
          <h3 className="font-black text-white text-xl italic uppercase tracking-tighter">سحب السيولة</h3>
          <button onClick={onClose} className="p-3 bg-white/10 rounded-2xl text-white hover:bg-white/20 transition-all"><X size={22} /></button>
        </div>
        <div className="bg-blue-600/5 border border-blue-500/10 p-8 rounded-[2.5rem] flex justify-between items-center flex-row-reverse shadow-inner">
           <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest leading-none">متاح للتسييل</span>
           <span className="text-4xl font-black text-blue-500 italic">{max.toFixed(2)} USDT</span>
        </div>
        ...
        <button onClick={submit} className="w-full bg-blue-600 text-white font-black py-8 rounded-[2.5rem] uppercase tracking-[0.6em] text-[14px] shadow-2xl hover:bg-blue-500 transition-all">تسييل فوري</button>
      </div>
    </div>
  );
};
    ...
    <div className={`min-h-screen bg-[#020617] p-10 flex flex-col justify-center ${lang === 'ar' ? 'rtl' : ''}`}>
      <div className="max-w-md mx-auto w-full space-y-14 animate-in fade-in duration-700">
        <div className="text-center space-y-6">
          <div className="w-28 h-28 bg-blue-600 rounded-[2.5rem] mx-auto flex items-center justify-center shadow-2xl rotate-12 hover:rotate-0 transition-all duration-500"><Zap size={64} className="text-white fill-white" /></div>
          <h1 className="text-6xl font-black italic tracking-tighter text-white">MINE<span className="text-blue-500">PRO</span></h1>
          <p className="text-[11px] font-black uppercase tracking-[0.6em] text-slate-800">V-Protocol Elite Liquidity</p>
        </div>
        <div className="bg-[#0b0f1a] border border-white/10 rounded-[4.5rem] p-14 shadow-2xl space-y-12 relative overflow-hidden">
          <div className="flex bg-[#020617] p-2 rounded-[2.2rem] border border-white/5 relative z-10 shadow-inner">
            <button onClick={() => setIsLogin(true)} className={`flex-1 py-5 rounded-[1.8rem] font-black text-xs transition-all uppercase tracking-widest ${isLogin ? 'bg-blue-600 text-white shadow-2xl' : 'text-slate-800'}`}>دخول</button>
            <button onClick={() => setIsLogin(false)} className={`flex-1 py-5 rounded-[1.8rem] font-black text-xs transition-all uppercase tracking-widest ${!isLogin ? 'bg-blue-600 text-white shadow-2xl' : 'text-slate-800'}`}>تسجيل</button>
          </div>
